...
#define SU_END { pSu->SendMessage(WM_ENABLECTRL, TRUE, IDC_SUBMIT); \
...
UINT SmThreadProc(LPVOID lpParam)
{
	...
	if (!std::regex_match(LPCTSTR(strEmail),
		std::wregex(L"[0-9a-z_\\-]+(\\.[0-9a-z_\\-]+)*@([0-9a-z_\\-]+\\.)+[a-z]+")))
	{
	...
	switch (pDlg->m_controller.signUp(
		LPCTSTR(strUsername),
		LPCTSTR(strPassword),
		LPCTSTR(strEmail),
		LPCTSTR(strRecommender),
		lszComputerName,
