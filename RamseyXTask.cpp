    ...
    if (cache.layer == 1)
    {
        ...
        for (t[1] = q[1] + 1; RX_MN - t[1] >= q[2] + q[3] + 2; ++t[1])
                ...
                ++t[2], ++currentComb)
			{
                ...
                if (currentComb < cache.combinationNum)
					continue;
...
COMB_FOUND:
    ...
    subBlockLength = combinationTable[RX_MN - t[1]][t[2]];
    blockLength = taskInfo[cache.combinationNum][RX_TASKINFO_BLOCKLENGTH];
    W1Size = taskInfo[cache.combinationNum][RX_TASKINFO_W1SIZE];
...
bool RamseyXTask::generateB1Only(unsigned int index, const RXFLAG &threadFlag)
{
    ...
    for (/*gen1[index] = gen1[index - 1] + gen1[1]*/;
        gen1[index] + gen1[1] * (t[1] - 1 - index) < RX_MN;
		++gen1[index])
        if ((restoringFromCheckPoint = !generateB1Only(index + 1, threadFlag)))
            return false;
...
std::wstring RamseyXTask::makeSpawnString(const RXTASKINFO &spawnInfo)
{
    static const unsigned long long totalCombNum = sizeof (taskInfo) / sizeof (taskInfo[0]);

    std::wstring result(std::to_wstring(spawnInfo.resultBits.count()));

    unsigned long long comb = spawnInfo.combinationNum, block = spawnInfo.block;
    for (unsigned int i = 0; i < RX_LAYER1_BLOCKS_PER_TASK; ++i, ++block)
    {
        ...
        {
            result += L',';
            result += std::to_wstring(comb);
            result += L',';
            result += std::to_wstring(block);
        }
    }
